...
async function savePassenger() {
    const response = await responsibleStore.createPassenger(form.value)
...
<template>
    <v-dialog v-model="isDialogOpen" max-width="700" scrim="rgba(0, 0, 30, 1)" height="auto">
        <v-card class="bg-white">
            ...
            <v-card-text>
                <p>Adicione os dados do passageiro para que possamos cadastrá-lo em nosso sistema</p>
                <v-container>
                    <v-row>
                        <v-col cols="12" md="6">
            ...
            <div class="edit-button full">
                ...
                <v-btn @click="savePassenger" color="primary" prepend-icon="mdi-content-save-outline" rounded="1">Salvar</v-btn>
            </div>
        </v-card>
    </v-dialog>
</template>

...
const route_id_driver = router.currentRoute.value.params.id;
...
onMounted(async () => {
  try {
    ...
    const drivers = goRoutesStore.state.transiDrivers
      .filter(d => d.my_location && d.my_location.latitude && d.my_location.longitude && d.driver_id == route_id_driver);
